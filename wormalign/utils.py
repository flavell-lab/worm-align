...
def extract_all_problems(
    ...
    write_to_json(
        ...
        "registration_problems_ALv8_swf360"
    )

def sample_swf360_problems(dataset):
    import random
    if dataset == "ALv6":
        dataset_name = "2022-03-30-01"
    elif dataset == "ALv7":
        dataset_name = "2022-03-30-02"
    elif dataset == "ALv8":
        dataset_name = "2022-03-31-01"

    file_name = f"registration_problems_{dataset}-swf360"
    with open(f"resources/{file_name}.json", "r") as f:
        problems = json.load(f)["train"][dataset_name]

    random.seed(42)
    sampled_problems = random.sample(problems, 3927)
    problem_dict = {"train": {dataset_name: sampled_problems}}

    with open(f"resources/{file_name}_sampled.json", "w") as f:
        json.dump(problem_dict, f, indent=4)
        print(f"{file_name}_sampled.json created!")

def subset_existing_h5(dataset):
    import h5py
    file_name = f"registration_problems_{dataset}-swf360_sampled"
    map_data = {"ALv6": "2022-03-30-01",
                "ALv7": "2022-03-30-02",
                "ALv8": "2022-03-31-01"}
    dataset_name = map_data[dataset]
    with open(f"resources/{file_name}.json", "r") as f:
        problems = json.load(f)["train"][dataset_name]

    for ch in ["ch1", "ch2"]:
        h5_path = \
        f"/data3/prj_register/{dataset}_swf360_{ch}/train/nonaugmented/{dataset_name}"
        for img in ["fixed_images", "moving_images"]:
            with h5py.File(f"{h5_path}/{img}_full.h5", "r") as f:
                with h5py.File(f'{h5_path}/{img}.h5', 'w') as f_subset:
                    for problem in problems:
                        f_subset.create_dataset(
                            problem,
                            data = f[problem][:]
                        )
            print(f"{img}.h5 created under {h5_path}")
